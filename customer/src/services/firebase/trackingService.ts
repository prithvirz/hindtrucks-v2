import type { ITrackingService, TripStatus, TripStatusCallback } from '../types'
import type { TripStep, TruckPosition, DriverInfo } from '@hindtrucks/shared'
import { db, loadDoc } from '@hindtrucks/shared/firebase'
import { getDoc, onSnapshot, type DocumentSnapshot, type DocumentData } from 'firebase/firestore'

const TRIP_DURATION = 6 * 60 * 60 * 1000

function toTripStatus(loadId: string, data: DocumentData): TripStatus {
...
export const firebaseTrackingService: ITrackingService = {
    async getTripStatus(loadId: string): Promise<TripStatus> {
        ...
        return toTripStatus(loadId, docSnap.data())
    },

    subscribeTripStatus(loadId: string, onUpdate: TripStatusCallback): () => void {
        return onSnapshot(loadDoc(db, loadId), (docSnap: DocumentSnapshot<DocumentData>) => {
            if (!docSnap.exists()) return
            onUpdate(toTripStatus(loadId, docSnap.data()))
        })
    },
}
